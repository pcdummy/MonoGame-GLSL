    ...
    public class QuadNode : QuadNodeCollection, IDisposable
    {
        ...
        private void CheckVertexAt(NodeVertex position, NodeContent flag, NodeSideVertex side)
        {
            if (this.IsDisabled(flag)//if the flag is not enabled
                && VertexTest(this.Vertices[(int)position].Value.Position, side, Camera.DefaultCamera.Position))//and the vertex can be enabled...
                this.EnableVertex(flag, position);
            else if (this.IsEnabled(flag)//if the flag is enabled
                && !VertexTest(this.Vertices[(int)position].Value.Position, side, Camera.DefaultCamera.Position))//and the vertex have to be disabled...
        ...
        {
            if (this.IsDisabled(flag)//if the flag is not enabled
                && ChildTest(dotprod, childBox, Camera.DefaultCamera.Position))//and the child bounding box show that the child have to be enabled
                ...
                && !ChildTest(dotprod, childBox, Camera.DefaultCamera.Position)) ////and the child bounding box show that the child have to be disabled
